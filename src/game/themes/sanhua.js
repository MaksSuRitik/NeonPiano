...
function drawGlassPlate(ctx, p, now, gw, gh, isT5, alphaMult = 1.0) {
  ...
  if (!p.gradNormal) {
    const gN = ctx.createLinearGradient(-sz * 0.5, -sz * 0.5, sz * 0.5, sz * 0.5);
    gN.addColorStop(0, `rgba(225, 242, 255, ${alpha * 0.58})`);
    gN.addColorStop(0.5, `rgba(125, 175, 218, ${alpha * 0.40})`);
    gN.addColorStop(1, `rgba(12, 26, 48, ${alpha * 0.68})`);
    p.gradNormal = gN;

    const gT = ctx.createLinearGradient(-sz * 0.5, -sz * 0.5, sz * 0.5, sz * 0.5);
    gT.addColorStop(0, `rgba(45, 10, 18, ${alpha * 0.70})`);
    gT.addColorStop(0.6, `rgba(20, 4, 8, ${alpha * 0.85})`);
    gT.addColorStop(1, `rgba(6, 1, 3, ${alpha * 0.90})`);
    p.gradT5 = gT;

    p.strokeRimNormal = `rgba(255, 255, 255, ${alpha * 0.92})`;
    p.strokeRimT5 = `rgba(255, 45, 85, ${alpha * 0.95})`;
    p.strokeShadowNormal = `rgba(8, 22, 45, ${alpha * 0.60})`;
    p.strokeShadowT5 = `rgba(12, 2, 4, ${alpha * 0.8})`;
    p.strokeCleavNormal = `rgba(255, 255, 255, ${alpha * 0.50})`;
    p.strokeCleavT5 = `rgba(255, 90, 120, ${alpha * 0.55})`;
  }
  ...
  ctx.strokeStyle = isT5 ? p.strokeRimT5 : p.strokeRimNormal;
  ...
  ctx.strokeStyle = isT5 ? p.strokeShadowT5 : p.strokeShadowNormal;
  ctx.lineWidth = 0.8;
  ctx.beginPath();
  ...
  if (p.verts.length >= 4) {
    ctx.strokeStyle = isT5 ? p.strokeCleavT5 : p.strokeCleavNormal;
    ctx.lineWidth = 0.7;
    ctx.beginPath();
...
function getVeilGradients(ctx, w, h, isEclipse) {
  ...
  let entry = veilGradCache.get(key);
  ...
  entry = {
    ...
    sheenStrokeUp: isEclipse ? `rgba(255, 101, 125, ${0.46 * alphaMult})` : `rgba(238, 54, 85, ${0.34 * alphaMult})`,
    pleatStroke: isEclipse ? `rgba(255, 101, 125, ${0.32 * alphaMult})` : `rgba(238, 54, 85, ${0.22 * alphaMult})`
  };
  if (veilGradCache.size >= 4) veilGradCache.clear();
...
function drawRedVeil(ctx, now, w, h, blend, layer) {
  ...
  if (layer === 0) {
    ...
    ctx.moveTo(p0_x, p0_y);
    ctx.bezierCurveTo(cp1_x, cp1_y, cp2_x, cp2_y, p1_x, p1_y);
    ctx.strokeStyle = vg.sheenStrokeUp;
    ctx.lineWidth = 1.3;
    ctx.stroke();
    ctx.restore();
...
export const SANHUA_THEME = {
  ...
  prewarm(w, headH, laneW, isLight = false, gw = 400, gh = 800) {
    if (gw > 0 && gh > 0) {
      try { ensureEnvCache(gw, gh); } catch (_) {}
    }
    const tiers = [0, 50, 100, 200, 400, 800];
    for (const t of tiers) {
      try {
        this._getHoldPaintCache(w, headH, t, false, isLight);
        this._getHoldPaintCache(w, headH, t, true, isLight);
        this._getHoldPaintCache(w, headH, t, false, !isLight);
        this._getHoldPaintCache(w, headH, t, true, !isLight);
      } catch (_) {}
    }
