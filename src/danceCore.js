...
const State = {
    ...
    holdsDropped: 0, // Підрахунок зірваних або пропущених довгих нот (для титулу "Король ночного дрифта" / "Кришталеве серце")
    totalHolds: 0,   // Загальна кількість довгих нот у поточному треку
    maxConsecutiveMisses: 0, // Пікова кількість промахів поспіль у сесії (для титулу "Танцующий на лезвии")
    survivedCritical: false, // Прапорець виживання на волосині від поразки
...
function bootGame() {
        ...
        if (userId && !cosmeticHistorySessions.has(userId)) {
            ...
            void Promise.allSettled([
                ...
                if (history.status === 'fulfilled') {
                    backfillHardcoreFromHistory(history.value);
                }
    ...
    function backfillHardcoreFromHistory(history = []) {
        if (!Array.isArray(history) || history.length === 0) return false;
        let modified = false;
        for (const match of history) {
            if (!match) continue;
            const title = match.trackTitle;
            if (!title) continue;
            const key = `neon_rhythm_${title}`;
            const raw = localStorage.getItem(key);
            if (!raw) continue;
            try {
                const record = JSON.parse(raw);
                let changed = false;
                const matchDiff = match.difficulty && ['easy', 'normal', 'hard'].includes(match.difficulty) ? match.difficulty : '';
                const matchHc = match.isHardcore === true || match.difficulty === 'hardcore';

                if (!Array.isArray(record.completedDifficulties)) {
                    record.completedDifficulties = record.difficulty ? [record.difficulty] : [];
                }

                if (matchDiff && !record.completedDifficulties.includes(matchDiff)) {
                    record.completedDifficulties.push(matchDiff);
                    changed = true;
                }

                if (matchHc) {
                    if (!record.hardcoreCompleted) { record.hardcoreCompleted = true; changed = true; }
                    if (!record.isHardcore) { record.isHardcore = true; changed = true; }
                    if (matchDiff === 'hard' || record.difficulty === 'hard' || record.completedDifficulties.includes('hard')) {
                        if (!record.hardHardcoreCompleted) { record.hardHardcoreCompleted = true; changed = true; }
                    }
                }

                if (!record.difficulty && matchDiff) {
                    record.difficulty = matchDiff;
                    changed = true;
                }
        ...
        if (modified && typeof renderSongList === 'function') {
            renderSongList();
        }
        return modified;
    }
...
// Головний ігровий цикл. Я викликаю його через requestAnimationFrame, що синхронізує оновлення логіки та рендеринг з частотою оновлення монітора.
    function gameLoop() {
        ...
        // Безпечна автопауза при затримках кадру понад 250мс (системна шторка, вхідний дзвінок, згортання, Hyprland робочі столи)
        // Надійно захищає рахунок від втрати та дає можливість гравцю спокійно відновити гру.
        if (dt > 250 && State.isPlaying && !State.isPaused) {
        ...
        // Перевірка фокусу вікна та видимості: автопауза при переході на іншу вкладку чи згортанні
        if ((!document.hasFocus() || document.hidden) && State.isPlaying && !State.isPaused) {
            togglePauseGame(true);
            State.lastFrameTime = 0;
            return;
        }
...
function handleInputDown(lane, touchY, touchX) {
    ...
    function missNote(tile, isSpawnedMiss) {
        ...
        if (tile) {
            tile.failed = true;
            tile.missed = true;
            if (tile.type === 'long') State.holdsDropped++;
        }
        State.consecutiveMisses++;
        State.maxConsecutiveMisses = Math.max(State.maxConsecutiveMisses || 0, State.consecutiveMisses);
        if (State.consecutiveMisses >= CONFIG.missLimit - 1) {
...
function updateRipples(dt) {
    ...
    async function startGame(idx) {
        ...
        analyzeAudio(song.audioUrl, mySession).then(generatedTiles => {
            ...
            if (generatedTiles) {
                ...
                State.totalHolds = generatedTiles.filter(t => t && t.type === 'long').length;
    ...
    renderMenu = function() {
        ...
        displayedSongs.forEach(({ song: s, originalIndex: i, saved }) => {
            ...
            const isSavedHardcore = saved.isHardcore === true || saved.hardcoreCompleted === true || (Array.isArray(saved.completedDifficulties) && saved.completedDifficulties.includes('hardcore'));
            const displaySaved = {
                ...saved,
                difficulty: diffKey || saved.difficulty,
                isHardcore: isSavedHardcore
            };
            const diffLabelMap = {
                easy: getText('diffEasy'),
                normal: getText('diffNormal'),
    ...
    function initControls() {
        ...
        const adminLevelSearch = document.getElementById('admin-level-search');
        ...
        function renderAdminLevelOptions(filterQuery = '') {
            ...
            if (currentVal && filtered.some(s => s.title === currentVal)) {
                adminSelectLevel.value = currentVal;
        ...
        async function initAdminScoresView() {
            ...
            // 1. Заповнюємо список рівнів з songsDB з урахуванням пошуку
        ...
        async function renderAdminLevelLeaderboard(trackTitle, useCache = false) {
            ...
            try {
                ...
                results.forEach((res, idx) => {
                    ...
                    const isSavedHc = res.isHardcore === true || res.hardcoreCompleted === true || (Array.isArray(res.completedDifficulties) && res.completedDifficulties.includes('hardcore'));
                    let effDiff = res.difficulty || '';
                    if (!effDiff && Array.isArray(res.completedDifficulties) && res.completedDifficulties.length > 0) {
                        let maxR = 0;
                        res.completedDifficulties.forEach(d => {
                            const r = getDiffRank(d);
                            if (r > maxR) { maxR = r; effDiff = d; }
                        });
                    }
                    const displayRes = {
                        ...res,
                        difficulty: effDiff || res.difficulty,
                        isHardcore: isSavedHc
                    };
                    const badgeHtml = (effDiff || isSavedHc)
                        ? `<span class="track-diff-badge ${effDiff}" style="font-size:0.7rem; padding:2px 7px; border-radius:4px; background:${diffColorMap[effDiff] || '#888'}22; color:${diffColorMap[effDiff] || '#888'}; border:1px solid ${diffColorMap[effDiff] || '#888'}44; display:inline-flex; align-items:center; gap:3px;">${isSavedHc ? icons.skull(11) : ''}${resultModeLabel(displayRes, getText)}</span>`
    ...
    function handleAutoPause(reason = 'blur') {
        forceReleaseAllInputs(); 
        if (State.isPlaying && !State.isPaused) {
            console.log(`[AutoPause] Triggered (${reason}). Pausing game and stopping audio.`);
            togglePauseGame(true); 
        }
    }

    // Обробник події blur (втрата фокусу вікном). Спрацьовує при вхідному дзвінку, натисканні Alt+Tab тощо.
    window.addEventListener('blur', () => handleAutoPause('window blur'));
    ...
    document.addEventListener('visibilitychange', () => {
        if (document.hidden) handleAutoPause('visibility hidden');
    });

    window.addEventListener('pagehide', () => handleAutoPause('pagehide'));

    // Watchdog таймер: виявляє зупинку презентаційного циклу композитора (наприклад, перемикання віртуального робочого столу на Hyprland / Wayland)
    // Коли браузер залишається "у фокусі", але rAF заморожується композитором, таймер негайно зупиняє аудіотрек і ставить гру на паузу.
    setInterval(() => {
        if (State.isPlaying && !State.isPaused) {
            const now = performance.now();
            const lastRaf = State.lastRafTime || now;
            if (now - lastRaf > 300) {
                handleAutoPause('watchdog compositor stall');
            }
        }
    }, 100);
    ...
    function resizeCanvas() { 
        ...
        const isSimulatedFs = document.body?.classList.contains('simulated-fullscreen') || document.documentElement?.classList.contains('simulated-fullscreen');
        const containerW = isSimulatedFs 
            ? (window.innerWidth || (gameContainer && gameContainer.clientWidth) || 480) 
            : ((gameContainer && gameContainer.clientWidth > 0) ? gameContainer.clientWidth : (window.innerWidth || 480));
        const containerH = isSimulatedFs 
            ? (window.visualViewport ? Math.round(window.visualViewport.height) : (window.innerHeight || 800)) 
            : ((gameContainer && gameContainer.clientHeight > 0) ? gameContainer.clientHeight : (window.innerHeight || 800));
        if (canvas) {
