...
export const i18n = {
  ...
  t(key, fallback = "") {
    ...
    const defaultFallbacks = {
      ...
      adminSpotifyLabel: currentLang === "RU" ? "Автозаполнение из Spotify:" : (currentLang === "EN" ? "Spotify Autofill:" : "Автозаповнення зі Spotify:"),
      ...
      adminSpotifySuccess: currentLang === "RU" ? "Данные трека успешно заполнены из Spotify!" : (currentLang === "EN" ? "Track metadata filled from Spotify successfully!" : "Дані треку успішно заповнено зі Spotify!"),
      adminSpotifyError: currentLang === "RU" ? "Не удалось получить данные из Spotify. Проверьте ссылку." : (currentLang === "EN" ? "Could not fetch Spotify track data. Please check link." : "Не вдалося отримати дані зі Spotify. Перевірте посилання."),
      adminSpotifyRedirectHint: currentLang === "RU" ? "Это ссылка Spotify! Метаданные заполнены. Для аудио укажите прямую ссылку на .mp3 файл (Catbox.moe)." : (currentLang === "EN" ? "This is a Spotify link! Metadata has been filled. For audio playback, provide a direct link (e.g. Catbox.moe)." : "Це посилання Spotify! Метадані заповнено. Для аудіо вкажіть пряме посилання на .mp3 файл (Catbox.moe)."),
